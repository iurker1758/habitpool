...
// dev proxy doesn't (DECISIONS.md #9 update; shape proven in umalab #53).
// API_ORIGIN is set on the Pages project, never in the repo: the tunnel
// hostname in production, a local backend under `wrangler pages dev`.
// The request is forwarded otherwise untouched, so the edge-attached
// Cf-Access-Jwt-Assertion rides along for the backend to verify (#10).
interface Env {
  API_ORIGIN?: string;
}
...
export async function onRequest({
  ...
  // builds absolute Locations from the rewritten URL (measured in umalab: the
